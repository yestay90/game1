...
local function messageListener( messageEvent )
    if messageEvent.data ~= nil and messageEvent.data.message ~= nil then
        ...
        if messageEvent.data.message == "connect" then
            ...
            {
                playerIDs = {composer.otherPlayerId},
                roomID = composer.matchId,
                message =  "connect"
            })
        else
            networkMove = messageEvent.data.message
            makeTurn(tonumber(networkMove))
            --native.showAlert("Message!",networkMove,{"OK"})
        end
    end
end
...
local function lunkaClick(event)
    ...
    if event.phase == "ended" then
        if (lunkaId<10) and (p1turn) and (composer.isHost==1) then
            makeTurn(tonumber(lunkaId))
            ---------"MESSAGING"----------------------------
            composer.gameNetwork.request("sendMessage", 
            {
                playerIDs = {composer.otherPlayerId},
        ...
        elseif (lunkaId>9) and (not p1turn) and (composer.isGuest==1) then
            makeTurn(tonumber(lunkaId))
            ---------"MESSAGING"----------------------------
            composer.gameNetwork.request("sendMessage", 
            {
...
function scene:hide( event )
    ...
    elseif ( phase == "did" ) then
        cleanBoard(sceneGroup)
        local function requestCallbackLeaveRoom( event )
        ...
        gameNetwork.request( "leaveRoom",
            {
                roomID = composer.matchId,
                listener = requestCallbackLeaveRoom  --including this will override any listener set in 'gameNetwork.setRoomListener()'
            }
        )
    end--OK
end
